...
class ManagerModel(nn.Module):
    ...
    def __init__(self, input_dim: int, worker_count: int, d_model: int = 512, 
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            batch_first=True,
            activation='gelu'  # use GELU to activate
        )
    ...
    def forward(self, src):
        ...
        Returns:
            worker selection prob: (batch_size, worker_count)
        """
        # encoder
...
def create_worker_pool(model_configs: List[Dict[str, Any]], available_models: Dict[str, Type[nn.Module]]) -> List[WorkerWrapper]:
    ...
    for config in model_configs:
        ...
        if model_type == 'lstm':
            ...
            model = TimeSeriesLSTM(input_dim, d_model, output_dim, num_layers, dropout).to(device)
