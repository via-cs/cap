...
import logging
from ..models.catf import ManagerModel, WorkerWrapper
...
class CATFTrainer:
    """
    Trainer class for Collaborative Adaptive Time-series Prediction (CATF) framework.
    ...
    def _compute_worker_weights(
        ...
        """
        Compute worker weights following the CATF paper's Equation (3).
    ...
    def train_step(
        ...
        # choose worker by trained manager
        with torch.no_grad():
    ...
    def train(
        ...
        val_loader: DataLoader,
        epochs: int,
        checkpoint_dir: str = "checkpoints/catf",
        min_lr: float = 1e-5,
        ...
        """
        General training loop for CATF model with multi-GPU support.
        ...
        if pre_training_epochs > 0 and self._is_main_process():
            ...
            print(f"\n{'='*60}")
            print(f"PRE-TRAINING COMPLETED. Starting CATF training...")
            print(f"{'='*60}\n")
    ...
    def save_test_loss(self, test_loss: float, log_dir: str = "logs", dataset_name: str = "unknown", model_config: str = "unknown", history: Dict = None):
        ...
        if self._is_main_process():
            ...
            with open(test_loss_file, 'w') as f:
                ...
                # Add basic worker rate information if available
                if history and 'worker_selections' in history and history['worker_selections']:
                    ...
                    if worker_selections:
                        ...
                        f.write(f"  Overall Rates:\n")
                        for i, rate in enumerate(overall_rates):
                            f.write(f"    Worker {i+1}: {rate:.6f} ({rate*100:.2f}%)\n")
                        
                        f.write(f"  Final Epoch Rates:\n")
                        for i, rate in enumerate(final_epoch_rates):
                            f.write(f"    Worker {i+1}: {rate:.6f} ({rate*100:.2f}%)\n")
                        
                        # Add summary statistics
                        f.write(f"  Summary:\n")
                        f.write(f"    Most Selected: Worker {overall_rates.argmax()+1} ({overall_rates.max()*100:.2f}%)\n")
                        f.write(f"    Least Selected: Worker {overall_rates.argmin()+1} ({overall_rates.min()*100:.2f}%)\n")
                        f.write(f"    Rate Range: {overall_rates.max()*100:.2f}% - {overall_rates.min()*100:.2f}%\n")
                        f.write(f"    Std Dev: {overall_rates.std()*100:.2f}%\n")
                
            print(f"Test loss saved to: {test_loss_file}")
            return test_loss_file
        return None 